...
void Shader::CreateFromFile(const char* vertexLocation, const char* framentLocation) {
	std::string vertexString = ReadFile(vertexLocation);
	std::string fragmentString = ReadFile(framentLocation);
...
void Shader::CompileShader(const char* vertexCode, const char* fragmentCode) {
	...
	for (size_t i = 0; i < MAX_POINT_LIGHTS; i++) {
		...
		uniformPointLight[i].uniformAmbientColour = glGetUniformLocation(shaderID, locBuff);
	...
	for (size_t i = 0; i < MAX_SPOT_LIGHTS; i++) {
		...
		uniformSpotLight[i].uniformAmbientColour = glGetUniformLocation(shaderID, locBuff);
...
void Shader::SetDirectionalLight(DirectionalLight* dLight){
	dLight->UseLight(
		...
		uniformDirectionalLight.uniformAmbientColour,
...
void Shader::SetPointLights(PointLight* pLight, unsigned int lightCount){
	...
	for (size_t i = 0; i < lightCount; i++) {
		pLight[i].UseLight(
			...
			uniformPointLight[i].uniformAmbientColour,
...
void Shader::SetSpotLights(SpotLight* sLight, unsigned int lightCount){
	...
	for (size_t i = 0; i < lightCount; i++) {
		sLight[i].UseLight(
			...
			uniformSpotLight[i].uniformAmbientColour,
